...
description: "基于 DDIA Chapter 8 的底层机制题完整解答"
---

# 设计故障检测与 Fencing

<div class="question-meta"><span>Ch8</span><span>底层机制题</span><span>中高级</span></div>

## 题干

设计集群成员故障检测、lease 授予和资源层 fencing 协议。

<ProgressToggle id="question-failure-detector-fencing" label="完成本题" />
...
- 规模基线：5000 节点、跨 3 可用区、心跳 1 秒、目标检测时间 < 10 秒。
...
以 `5000 节点、跨 3 可用区、心跳 1 秒、目标检测时间 < 10 秒` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：Heartbeat(node,incarnation)；Acquire(resource) 返回 token；Write(resource,token)。

**核心数据**：member(node,incarnation,last_seen,status)；lease(resource,holder,token,expires_at)。
...
1. **Gossip detector 提供 suspicion；quorum 协调器发 lease/token；每个存储资源持久化 max_token**
...
更激进检测缩短恢复但增加误判；suspicion 阶段可吸收短暂抖动。
