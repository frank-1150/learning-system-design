...
description: "基于 DDIA Chapter 1 的产品系统题完整解答"
---

# 设计短链服务

<div class="question-meta"><span>Ch1</span><span>产品系统题</span><span>中高级</span></div>

## 题干

设计一个全球可用的短链创建与跳转服务，重点保证跳转低延迟和链接持久性。

<ProgressToggle id="question-url-shortener" label="完成本题" />
...
- 规模基线：1 亿月活、每日 2 亿次跳转、读写比约 100:1、P99 < 100ms。
...
以 `1 亿月活、每日 2 亿次跳转、读写比约 100:1、P99 < 100ms` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：POST /links 创建短链；GET /{code} 返回 301/302；DELETE /links/{code} 撤销。

**核心数据**：link(code PK, target_url, owner_id, created_at, expires_at, status)。
...
随机 ID 简单但需处理碰撞；发号器无碰撞但引入协调和可预测性。
