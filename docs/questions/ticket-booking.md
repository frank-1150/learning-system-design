...
description: "基于 DDIA Chapter 7 的产品系统题完整解答"
---

# 设计票务预订与支付事务

<div class="question-meta"><span>Ch7</span><span>产品系统题</span><span>中高级</span></div>

## 题干

设计演唱会座位选择、短时锁定、支付和超时释放。

<ProgressToggle id="question-ticket-booking" label="完成本题" />
...
- 规模基线：开票峰值 100 万请求/秒、10 万座位、锁定 5 分钟、绝不重复售票。
...
以 `开票峰值 100 万请求/秒、10 万座位、锁定 5 分钟、绝不重复售票` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：POST /holds；POST /holds/{id}/confirm；POST /payments/webhook；GET /orders/{id}。

**核心数据**：seat(event_id, seat_id, state, version)；hold(id, expires_at, state)；order(id, hold_id, payment_state)。
...
严格排队降低峰值吞吐但保护库存正确性；乐观重试吞吐高但冲突时体验差。
