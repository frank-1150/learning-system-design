...
description: "基于 DDIA Chapter 2 的生产事故题完整解答"
---

# 修复反规范化一致性事故

<div class="question-meta"><span>Ch2</span><span>生产事故题</span><span>中高级</span></div>

## 题干

用户改名后订单、评论和搜索结果显示三个不同名字，批量修复又压垮主库。

<ProgressToggle id="question-denormalization-incident" label="完成本题" />
...
- 规模基线：10 亿份冗余文档、每日 100 万次资料变更、修复扫描产生 4GB/s 读流量。
...
以 `10 亿份冗余文档、每日 100 万次资料变更、修复扫描产生 4GB/s 读流量` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：ProfileChanged(user_id, version, fields)；GET /repair/status；POST /repair/ranges。

**核心数据**：profile 为事实源；投影保存 source_version；repair_job 记录范围、水位和重试。
...
读取时 join 保证新鲜但增加耦合；反规范化提高读性能但必须建设可靠传播和对账。
