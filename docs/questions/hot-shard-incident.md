...
description: "基于 DDIA Chapter 6 的生产事故题完整解答"
---

# 处理热点分片与扩容雪崩

<div class="question-meta"><span>Ch6</span><span>生产事故题</span><span>中高级</span></div>

## 题干

直播事件让单个分片达到平时 80 倍流量，自动扩容同时触发大量迁移并拖慢整个集群。

<ProgressToggle id="question-hot-shard-incident" label="完成本题" />
...
- 规模基线：热点 300k QPS、单分片上限 25k、迁移占 70% 网络、错误率 18%。
...
以 `热点 300k QPS、单分片上限 25k、迁移占 70% 网络、错误率 18%` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：hot_key dashboard；PauseRebalance；OverrideRoute(key,salt,ttl)。

**核心数据**：hotspot(key, qps, shard, detected_at)；override(key, subshards, expires_at)。
...
局部特殊处理会增加路由复杂度，但比为一个热点移动全局数据更可控。
