...
description: "基于 DDIA Chapter 4 的生产事故题完整解答"
---

# 处理滚动发布反序列化故障

<div class="question-meta"><span>Ch4</span><span>生产事故题</span><span>中高级</span></div>

## 题干

新 producer 发布后 8% 的旧消费者进入毒消息重启循环，队列积压迅速增长。

<ProgressToggle id="question-serialization-rollout-incident" label="完成本题" />
...
- 规模基线：500 个 consumer 实例、积压每分钟增加 300 万条、最大事件 2MB。
...
以 `500 个 consumer 实例、积压每分钟增加 300 万条、最大事件 2MB` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：DLQ(topic, partition, offset, schema_id, error)；兼容性审计查询 producer/consumer version。

**核心数据**：deployment(version, schema_range)；consumer_capability(group, max_schema_id)；poison_event 记录失败。
...
- Consumer capability inventory
- DLQ 与跳过策略
- Schema gate 和 canary topic
...
跳过消息可恢复吞吐但可能破坏状态；停分区保护正确性但扩大业务不可用。
