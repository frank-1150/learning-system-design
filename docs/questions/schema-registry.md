...
description: "基于 DDIA Chapter 4 的底层机制题完整解答"
---

# 设计 Schema Registry

<div class="question-meta"><span>Ch4</span><span>底层机制题</span><span>中高级</span></div>

## 题干

设计高可用 Schema Registry，阻止不兼容格式进入生产。

<ProgressToggle id="question-schema-registry" label="完成本题" />
...
- 规模基线：10 万 subject、500 万 schema 版本、发布峰值 1000 次/秒、读取 20 万次/秒。
...
以 `10 万 subject、500 万 schema 版本、发布峰值 1000 次/秒、读取 20 万次/秒` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：Register(subject,schema,mode)；CheckCompatibility；GetById；ListVersions。

**核心数据**：schema(id, fingerprint, canonical_text)；subject_version(subject, version, schema_id, state)。
...
1. **无状态 API**
2. **强一致元数据存储**
3. **缓存/CDN；CI 与 producer SDK 在发布前查询**
...
- Canonicalization 与 fingerprint
- 兼容规则图
- 版本状态机和不可复用 ID
...
中心 Registry 提供统一治理但成为发布依赖；客户端缓存提高可用性但必须有明确过期策略。
...
本题主要对应 Chapter 4 **Encoding and Evolution**，源 PDF 第 133-166 页。答案结合了原书概念与原创生产化设计，不代表原书提供了该题的唯一解法。
