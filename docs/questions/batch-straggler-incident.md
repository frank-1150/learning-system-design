...
description: "基于 DDIA Chapter 10 的生产事故题完整解答"
---

# 处理 Straggler 与重复输出

<div class="question-meta"><span>Ch10</span><span>生产事故题</span><span>中高级</span></div>

## 题干

99.8% 任务已完成但少数 reducer 持续 4 小时，推测执行又生成重复账单文件。

<ProgressToggle id="question-batch-straggler-incident" label="完成本题" />
...
- 规模基线：5 万 reducer、100 个热 key、重复输出金额 800 万、shuffle 40PB。
...
以 `5 万 reducer、100 个热 key、重复输出金额 800 万、shuffle 40PB` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：查询 task progress/key histogram；暂停 publish；按 attempt token 审计输出。

**核心数据**：task_attempt(id,logical_task,token,state)；output_partition(version,partition,winning_token)。
...
推测执行缩短硬件长尾，但对数据倾斜无效且会增加资源与副作用风险。
