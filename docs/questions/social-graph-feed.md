...
description: "基于 DDIA Chapter 2 的产品系统题完整解答"
---

# 设计社交关系与 Feed 数据层

<div class="question-meta"><span>Ch2</span><span>产品系统题</span><span>中高级</span></div>

## 题干

设计关注关系、用户主页和首页 Feed 的数据层，支持大 V 与普通用户。

<ProgressToggle id="question-social-graph-feed" label="完成本题" />
...
- 规模基线：5 亿用户、日活 1 亿、峰值读 100 万 QPS、单个大 V 1 亿粉丝。
...
以 `5 亿用户、日活 1 亿、峰值读 100 万 QPS、单个大 V 1 亿粉丝` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：POST /follow；DELETE /follow；GET /feed?cursor=；GET /users/{id}/followers。

**核心数据**：edge(follower_id, followee_id, created_at)；post(id, author_id, created_at)；feed_item(user_id, rank, post_id)。
...
写扩散降低读延迟但成本随粉丝数增长；读扩散节省写入但首页组装更慢。
