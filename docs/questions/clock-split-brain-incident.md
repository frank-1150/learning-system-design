...
description: "基于 DDIA Chapter 8 的生产事故题完整解答"
---

# 处理时钟漂移与 Split-Brain

<div class="question-meta"><span>Ch8</span><span>生产事故题</span><span>中高级</span></div>

## 题干

NTP 故障和长 GC 让两个控制器都认为自己持有锁，并分别向存储写入配置。

<ProgressToggle id="question-clock-split-brain-incident" label="完成本题" />
...
- 规模基线：两控制器时钟相差 90 秒、GC pause 45 秒、错误配置传播到 30% 节点。
...
以 `两控制器时钟相差 90 秒、GC pause 45 秒、错误配置传播到 30% 节点` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：冻结控制面写；查询 lease token、term、config revision；按单调 revision 回滚。

**核心数据**：config(revision,term,payload,status)；lease(holder,token)；audit(actor,token,operation)。
...
- Monotonic clock 仅测时长
- Quorum authority
- Term/revision fencing
...
停写恢复牺牲短期可用性但保护控制面正确性；继续双主会让损害不可界定。
