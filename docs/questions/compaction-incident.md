...
description: "基于 DDIA Chapter 3 的生产事故题完整解答"
---

# 排查 Compaction 尾延迟事故

<div class="question-meta"><span>Ch3</span><span>生产事故题</span><span>中高级</span></div>

## 题干

写入高峰时磁盘带宽打满，L0 文件持续增长，点查 P99 周期性超过 3 秒。

<ProgressToggle id="question-compaction-incident" label="完成本题" />
...
- 规模基线：写 150MB/s、compaction 350MB/s、磁盘上限 450MB/s、L0 从 8 增至 120 个文件。
...
以 `写 150MB/s、compaction 350MB/s、磁盘上限 450MB/s、L0 从 8 增至 120 个文件` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：暴露 level_bytes、pending_compaction_bytes、stall_seconds、read_amplification。

**核心数据**：compaction_task(input_files, output_level, bytes, status)；记录每层文件和 key range。
...
1. **前台读写与后台压实共享 I/O scheduler；先限入站写并提高 L0 紧急度，再重新配置层级和带宽**
...
降低压实会暂时保护写延迟但积累读放大；加速压实会抢占前台 I/O，需动态预算。
