...
description: "基于 DDIA Chapter 9 的生产事故题完整解答"
---

# 处理 2PC 协调者故障

<div class="question-meta"><span>Ch9</span><span>生产事故题</span><span>中高级</span></div>

## 题干

跨库转账在 prepare 后协调者磁盘损坏，参与者持锁等待，交易队列逐步停摆。

<ProgressToggle id="question-two-phase-commit-incident" label="完成本题" />
...
- 规模基线：8 个分片、4 万 in-doubt 事务、锁等待 P99 60 秒、无可用协调者副本。
...
以 `8 个分片、4 万 in-doubt 事务、锁等待 P99 60 秒、无可用协调者副本` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：ListPrepared(txn_id)；RecoverDecision(txn_id)；FenceCoordinator(epoch)。

**核心数据**：transaction(txn_id,participants,state,decision,coordinator_epoch)；participant prepared_lsn。
...
保持阻塞保护原子性但降低可用性；启发式中止恢复业务却可能产生需人工修复的不一致。
