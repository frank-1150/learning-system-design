...
description: "基于 DDIA Chapter 10 的产品系统题完整解答"
---

# 设计批量网页抓取与索引流水线

<div class="question-meta"><span>Ch10</span><span>产品系统题</span><span>中高级</span></div>

## 题干

设计可重跑、去重并每天构建新搜索索引的批处理平台。

<ProgressToggle id="question-batch-crawl-index" label="完成本题" />
...
- 规模基线：100 亿 URL、每日抓取 5PB、10 万 worker、索引发布时间 < 12 小时。
...
以 `100 亿 URL、每日抓取 5PB、10 万 worker、索引发布时间 < 12 小时` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：SubmitSnapshot(seed_version)；GetJobStatus；PublishIndex(version)；robots fetch。

**核心数据**：url_record(url_hash,status,etag,last_fetch)；dataset(version,partitions,checksum)；job_attempt。
...
全量构建简单可验证但成本高；增量构建更快却需要处理删除和版本依赖。
