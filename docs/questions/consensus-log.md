...
description: "基于 DDIA Chapter 9 的底层机制题完整解答"
---

# 设计共识日志与 Leader Election

<div class="question-meta"><span>Ch9</span><span>底层机制题</span><span>中高级</span></div>

## 题干

设计基于 term、quorum 和复制日志的容错共识模块。

<ProgressToggle id="question-consensus-log" label="完成本题" />
...
- 规模基线：5 节点、容忍 2 故障、日志每秒 2 万条、快照 1GB。
...
以 `5 节点、容忍 2 故障、日志每秒 2 万条、快照 1GB` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：RequestVote(term,last_index,last_term)；AppendEntries；InstallSnapshot。

**核心数据**：persistent current_term/voted_for/log；volatile commit_index/next_index/match_index。
...
更长选举超时稳定但故障切换慢；更短超时恢复快却对网络抖动敏感。
