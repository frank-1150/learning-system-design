...
description: "基于 DDIA Chapter 3 的底层机制题完整解答"
---

# 设计 LSM-Tree KV Store

<div class="question-meta"><span>Ch3</span><span>底层机制题</span><span>中高级</span></div>

## 题干

设计支持持久写入、点查、范围扫描和崩溃恢复的 LSM 存储引擎。

<ProgressToggle id="question-lsm-kv-store" label="完成本题" />
...
- 规模基线：单节点 2TB、写入 20 万 ops/s、P99 点查 < 10ms、对象均值 1KB。
...
以 `单节点 2TB、写入 20 万 ops/s、P99 点查 < 10ms、对象均值 1KB` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：Put(key,value,seq)；Get(key,snapshot)；Scan(start,end,snapshot)；Delete(key)。

**核心数据**：WAL record；Memtable skiplist；SSTable data/index/filter blocks；manifest 保存文件与 level。
...
4. **flush queue**
5. **L0 SSTable**
6. **leveled compaction；读取合并内存和各层**
...
- Sequence number 与 tombstone
- Bloom Filter/block cache
- Leveled compaction 与 snapshot
...
Size-tiered 写放大较低但读放大高；leveled 读稳定但 compaction 写放大更大。
