...
description: "基于 DDIA Chapter 4 的产品系统题完整解答"
---

# 设计可演进的公共 API 与事件格式

<div class="question-meta"><span>Ch4</span><span>产品系统题</span><span>中高级</span></div>

## 题干

设计跨团队使用十年的订单 API 与事件契约，允许独立发布和回放历史数据。

<ProgressToggle id="question-evolvable-api-events" label="完成本题" />
...
- 规模基线：200 个消费者、每日 20 亿事件、历史保留 7 年、每周数十次 schema 变更。
...
以 `200 个消费者、每日 20 亿事件、历史保留 7 年、每周数十次 schema 变更` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：POST /orders v2；OrderCreated schema；GET /schemas/{subject}/versions。

**核心数据**：字段使用稳定编号；事件含 event_id、schema_version、occurred_at、aggregate_version。
...
新增事件类型更清晰但增加消费者迁移；在旧事件加可选字段更平滑但容易积累含糊语义。
