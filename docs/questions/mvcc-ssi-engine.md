...
description: "基于 DDIA Chapter 7 的底层机制题完整解答"
---

# 设计 MVCC 与 SSI

<div class="question-meta"><span>Ch7</span><span>底层机制题</span><span>中高级</span></div>

## 题干

为关系数据库设计快照可见性、垃圾回收和 Serializable Snapshot Isolation。

<ProgressToggle id="question-mvcc-ssi-engine" label="完成本题" />
...
- 规模基线：1TB 热数据、10 万事务/秒、95% 读、最长快照 30 分钟。
...
以 `1TB 热数据、10 万事务/秒、95% 读、最长快照 30 分钟` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：Begin() 返回 snapshot；Read(key,snapshot)；Commit(read_set,write_set)。

**核心数据**：version(key, xmin, xmax, value)；transaction(id, state, snapshot)；rw_dependency(from,to)。
...
1. **Transaction Manager 分配时间戳；版本链服务快照读；Commit 检测 ww 冲突和危险 rw 结构；GC 清理不可见版本**
...
精确谓词锁更少误中止但实现复杂；粗粒度范围跟踪简单却降低并发。
