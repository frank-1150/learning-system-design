...
description: "基于 DDIA Chapter 5 的底层机制题完整解答"
---

# 设计 Leaderless Quorum KV Store

<div class="question-meta"><span>Ch5</span><span>底层机制题</span><span>中高级</span></div>

## 题干

设计 Dynamo 风格的无主复制 KV Store，支持节点故障下读写。

<ProgressToggle id="question-leaderless-quorum-kv" label="完成本题" />
...
- 规模基线：100 节点、RF=3、100 万 ops/s、对象 4KB、99.99% 可用。
...
以 `100 节点、RF=3、100 万 ops/s、对象 4KB、99.99% 可用` 为容量基线。先按峰均比 5 倍预留入口吞吐，再计算 `日增量 = 峰值写入 × 平均对象大小 × 有效写入秒数`。副本、索引、WAL/日志和压缩前空间分别计入，生产容量至少保留 30% 运维余量。
...
**核心接口**：Put(key,value,context) 返回 version；Get(key) 返回 sibling versions；Repair(key)。

**核心数据**：value(key, version_vector, payload, tombstone, timestamp)；hint(target_node, mutation)。
...
1. **Coordinator**
2. **preference list 上 N 个副本；并行等待 W 写确认或 R 读响应；后台 repair**
...
较小 R/W 提高可用性和延迟，但增加读到旧值和冲突合并概率。
